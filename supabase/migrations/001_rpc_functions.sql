-- ============================================
-- ROOMZ - Supabase RPC Functions Migration
-- ============================================
...
CREATE OR REPLACE FUNCTION increment_favorite_count(room_id UUID)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION decrement_favorite_count(room_id UUID)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
CREATE POLICY "Landlords can view room bookings" ON bookings
  FOR SELECT
  USING (auth.uid()::text = landlord_id::text);
...
CREATE POLICY "Landlords can update bookings" ON bookings
  FOR UPDATE
  USING (auth.uid()::text = landlord_id::text);

-- Policy: Tenants can cancel their own bookings
DROP POLICY IF EXISTS "Tenants can cancel bookings" ON bookings;
CREATE POLICY "Tenants can cancel bookings" ON bookings
  FOR UPDATE
  USING (auth.uid()::text = tenant_id::text);
...
SELECT 'RoomZ RPC Functions và RLS Policies đã tạo thành công! Nhớ tạo Storage policies qua Dashboard.' as result;
