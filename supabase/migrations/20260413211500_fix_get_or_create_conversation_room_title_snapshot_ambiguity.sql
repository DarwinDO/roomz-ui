...
BEGIN
  ...
  lock_key :=
    ...
    END
    || ':' || COALESCE(get_or_create_conversation.room_id::TEXT, 'no-room');
  ...
  IF conversation_id IS NULL THEN
    ...
    VALUES (
      get_or_create_conversation.room_id,
      get_or_create_conversation.room_title_snapshot
    )
  ...
  ELSIF get_or_create_conversation.room_title_snapshot IS NOT NULL THEN
    UPDATE public.conversations
    SET room_title_snapshot = COALESCE(
      public.conversations.room_title_snapshot,
      get_or_create_conversation.room_title_snapshot
    ),
        updated_at = NOW()
