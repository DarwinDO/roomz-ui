-- ============================================
-- Normalize room locations and extend search_rooms with geo filters
-- Date: 2026-03-10
-- ============================================
...
AS $$
    SELECT CASE
        ...
        ELSE 6371.0 * 2.0 * asin(
            sqrt(
                power(sin(radians(((p_lat2 - p_lat1) / 2.0)::double precision)), 2)
                + cos(radians((p_lat1)::double precision))
                * cos(radians((p_lat2)::double precision))
                * power(sin(radians(((p_lng2 - p_lng1) / 2.0)::double precision)), 2)
            )
        )
    END;
$$;
...
UPDATE public.rooms
SET
    address = NULLIF(regexp_replace(btrim(address), '\s+', ' ', 'g'), ''),
    district = NULLIF(regexp_replace(btrim(district), '\s+', ' ', 'g'), ''),
    city = CASE
        WHEN city IS NULL THEN NULL
        WHEN lower(btrim(city)) IN ('hà nội', 'ha noi', 'hanoi', 'tp hà nội', 'tp ha noi', 'thành phố hà nội', 'thanh pho ha noi') THEN 'Thành phố Hà Nội'
        WHEN lower(btrim(city)) IN ('hồ chí minh', 'ho chi minh', 'ho chi minh city', 'tp hồ chí minh', 'tp ho chi minh', 'thành phố hồ chí minh', 'thanh pho ho chi minh', 'tphcm', 'sài gòn', 'sai gon') THEN 'Thành phố Hồ Chí Minh'
        WHEN lower(btrim(city)) IN ('đà nẵng', 'da nang', 'tp đà nẵng', 'tp da nang', 'thành phố đà nẵng', 'thanh pho da nang') THEN 'Thành phố Đà Nẵng'
        WHEN lower(btrim(city)) IN ('cần thơ', 'can tho', 'tp cần thơ', 'tp can tho', 'thành phố cần thơ', 'thanh pho can tho') THEN 'Thành phố Cần Thơ'
        ELSE NULLIF(regexp_replace(btrim(city), '\s+', ' ', 'g'), '')
    END
WHERE address IS NOT NULL OR district IS NOT NULL OR city IS NOT NULL;
...
GRANT EXECUTE ON FUNCTION public.calculate_distance_km(numeric, numeric, numeric, numeric) TO authenticated;
GRANT EXECUTE ON FUNCTION public.calculate_distance_km(numeric, numeric, numeric, numeric) TO anon;
