...
export function createInitialWorkspaceState(viewerMode: RomiViewerMode): RomiWorkspaceState {
  return {
    ...
    journeyState: {
      stage: "discover",
      summary: `${ROMI_NAME} đang chờ bạn mô tả nhu cầu.`,
    },
    clarification: null,
    handoff: null,
  };
...
export function applyStreamEvent(
  ...
  if (event.type === "final") {
    ...
    return {
      ...
      streamStatus: null,
      clarification: event.metadata?.clarification ?? state.clarification,
      handoff: event.metadata?.handoff ?? state.handoff,
  ...
  switch (action.type) {
    case "bootstrap":
      return {
        ...
        messages: action.messages.length ? action.messages : [createWelcomeMessage(action.viewerMode)],
        streamStatus: null,
